...
public class Level2 extends BaseScreen{
	...
	public void update(float delta) {
		...
		if(gameOver) {
			...
			for(BaseActor bomb : BaseActor.getList(mainStage, BombEnemy.class)) {
				bomb.setSpeed(0);
			}
		...
		if(BaseActor.count(mainStage, Beetle.class) == 0 && BaseActor.count(mainStage, Keaton.class) == 0 && BaseActor.count(mainStage, BombEnemy.class) == 0&& BaseActor.count(mainStage, Skeleton.class) == 0) {
			youWin = true;
		}
		...
		if(!paused) {
			...
			if(Gdx.input.isKeyJustPressed(Keys.ESCAPE)) {
				...
				for(BaseActor bombActor : BaseActor.getList(mainStage, BombEnemy.class)) {
					BombEnemy bomb = (BombEnemy)bombActor;
					if(bomb.isHit()) {
						bomb.setSpeed(75);
					}else {
						bomb.setSpeed(25);
					}
					bomb.setAnimationPaused(false);
		...
		for(BaseActor solid : BaseActor.getList(mainStage, Solid.class)) {
			...
			for(BaseActor bomb : BaseActor.getList(mainStage, BombEnemy.class)) {
				...
				if(bomb.isWithinDistance(0.1f, solid)) {
					bomb.setMotionAngle(bomb.getMotionAngle() + angulo);
				}
			}
		...
		/**
		 * Interaccion con los huesos
		 */
		for(BaseActor boneActor : BaseActor.getList(mainStage, Bone.class)) {
			Bone bone = (Bone) boneActor;
			if(player.overlaps(bone)) {
				health--;
				linkHitSound.play();
				bone.remove();
			}
		...
		/**
		 * Interaccion con el enemigo Bomba
		 */
		for(BaseActor bombActor : BaseActor.getList(mainStage, BombEnemy.class)) {
			player.preventOverlap(bombActor);
		}
		...
		for(BaseActor npcActor : BaseActor.getList(mainStage, NPC.class)) {
			...
			if(nearby && !npc.isViewing()) {
				...
				if(npc.getID().equals("shopKeeper2")) {
					girlLaugh.play();
		...
		if(sword.isVisible()) {
			...
			for(BaseActor bombActor : BaseActor.getList(mainStage, BombEnemy.class)) {
				BombEnemy bomb = (BombEnemy)bombActor;
				if(sword.overlaps(bomb)) {
					bomb.preventOverlap(sword);
					Vector2 swordPosition = new Vector2(sword.getX(), sword.getY());
					Vector2 bombPosition = new Vector2(bomb.getX(), bomb.getY());
					Vector2 hitVector = swordPosition.sub(bombPosition);
					bomb.setMotionAngle(hitVector.angle());
					if(bomb.isHit() == false) {
						bomb.setHit(true);
					}
	...
	private void mapInit() {
		TilemapActor tma = new TilemapActor("assets/maps/level2/level2.tmx", mainStage);
		...
		for(MapObject obj : tma.getRectangleList("bomb")) {
			MapProperties prop = obj.getProperties();
			BombEnemy bomb = new BombEnemy((float)prop.get("x"), (float)prop.get("y"), mainStage);
			bomb.setPosX((float)prop.get("x"));
			bomb.setPosY((float)prop.get("y"));
		}
		
		for(MapObject obj : tma.getRectangleList("keaton")) {
			MapProperties prop = obj.getProperties();
			Keaton keaton = new Keaton((float)prop.get("x"), (float)prop.get("y"), mainStage);
			keaton.setPosX((float)prop.get("x"));
			keaton.setPosY((float)prop.get("y"));
		}
		
		for(MapObject obj : tma.getRectangleList("skeleton")) {
			MapProperties prop = obj.getProperties();
			Skeleton skeleton = new Skeleton((float)prop.get("x"), (float)prop.get("y"), mainStage);
			skeleton.setPosX((float)prop.get("x"));
			skeleton.setPosY((float)prop.get("y"));
		}
	...
	private void soundtrackInit() {
		backgroundMusic = Gdx.audio.newMusic(Gdx.files.internal("assets/music/47 Royal Crypt.mp3"));
		...
		girlLaugh = Gdx.audio.newSound(Gdx.files.internal("assets/sounds/converted/converted_MC_Unheard_Girl_Laugh2.wav"));
